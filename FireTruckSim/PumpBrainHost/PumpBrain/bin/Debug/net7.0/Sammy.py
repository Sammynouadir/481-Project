    ...
    def __init__(self, root):
        ...
        def toggle_hydrant():
            ...
            if self.cp_interface.GetHydrantConnected():
                ...
                self.cp_interface.SetHydrantConnected(False)
                print("New Hydrant State after disconnecting:", self.cp_interface.GetHydrantConnected())
            else:
                print("Connecting hydrant...")
                self.cp_interface.SetHydrantConnected(True)
                print("New Hydrant State after connecting:", self.cp_interface.GetHydrantConnected())
            self.root.after(500, self.update_ui)  # Delay UI update to allow state change
